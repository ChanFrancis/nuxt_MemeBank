<template>
  <div class="card bg-base-200 shadow-xl p-6 w-[280px] mx-auto">
    <h2 class="card-title text-xl mb-4">Poster un meme</h2>
    <input
        type="file"
        accept="image/*"
        @change="onFileChange"
    ...
    <div v-if="error" class="alert alert-error mt-4 p-2 text-sm">
      Erreur : {{ error.message }}
    </div>

    <div v-if="success && !error" class="alert alert-success mt-4 p-2 text-sm">
      Meme upload !
    </div>
  </div>
</template>
