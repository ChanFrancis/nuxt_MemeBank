...
export function useImageUploader(uploadUrl = "/api/upload") {
    ...
    async function upload(tags: string[], modalRef: { open: () => void; close: () => void } | null): Promise<void> {
        if (!file.value) {
        ...
        try {
            ...
            error.value = err
            toast.error("Échec de l’ajout du meme.")
        } finally {
            loading.value = false

            // refresh stickers and close modal

            const storeStickers = useStickerStore()
            await storeStickers.fetchStickers()

            toast.success("Meme ajouté avec succès !")
